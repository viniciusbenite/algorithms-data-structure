...
//
// Versão antiga e que demora mais tempo a executar
...
static int min_tour[max_n_cities + 1],max_tour[max_n_cities + 1],tour[max_n_cities+1];
static long n_tours;
...
void tsp_v1(int n,int m,int *a)
{
  ...
  { // visit permutation
    ...
    for(i = 0;i < n;i++){
      ...
      tour[i]=a[i];     /*Este array regista o numero da cidade onde o tour está a passar-->   a[i] = nºcidade*/
    ...
    if (distance > max_length){
      ...
      dados[n].max_distance = distance;	/*Update da distancia maxima para um tour com n cidades*/

      memcpy(max_tour, tour, sizeof(tour));   /*esta função copia o conteudo do tour para o conteudo do max_tour, ficando assim o array max_tour com o caminho mais longo*/
      for (i = 0; i < n; i++) {
        dados[n].max_tour_str[i] =  tour[i];	/*guarda os valores do max_tour no struct dados*/
      }
    }
    ...
    if (distance < min_length){ 
      ...
      dados[n].min_distance = distance;		/*Update da distancia minima para um tour com n cidades*/

      memcpy(min_tour, tour, sizeof(tour));     /*copia o conteudo do tour para min_tour*/
      
      for (i = 0; i < n; i++) {
        dados[n].min_tour_str[i] =  tour[i];	/*guarda os valores do min_tour no struct dados*/
      }
}
...
int main(int argc,char **argv)
{
  ...
  print_distances();
#endif
  for(n = 3;n <= (max_n_cities-5);n++)	/*AUMENTAR OU DIMINUIR--MAXIMO 18 CIDADES*/
  {
    //
    // try tsp_v1
    //
