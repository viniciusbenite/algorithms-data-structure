...
struct tree_node** createQueue(int *front, int *rear) 
{ 
    struct node **queue = 
...
void enQueue(struct tree_node **queue, int *rear, struct tree_node *new_node) 
{ 
    queue[*rear] = new_node; 
    (*rear)++; 
} 
  
struct tree_node *deQueue(struct tree_node **queue, int *front) 
{ 
    (*front)++; 
...
static int count_leaves(tree_node *link)
{
  ...
  if (link == NULL) { //Checar se a arvore esta vazia
    return 0;
  }
...
static int tree_height(tree_node *link)
{
  ...
  {
    int left_height = tree_height(link -> left); // Calcular a altura de cada sub arvore
    int right_height = tree_height(link -> right);
...
static int count_function_calls_on_hit(tree_node *link,int level)
{
    ...
    if (search_tree(link, link->data) != NULL)  // Caso o dado seja encontrado, o custo sera a
    {                                           // altura da arvore, no caso: level + 1
        hit += level+1;       
    }

    if (link->left && link->right)                          // Funciona recursivamente. Checa se
    {                                                       // o node possui filhos e desce 1 level
    ...
    search_counter--;           // Como estamos chamando a funcao search_tree, o search_counter
    return hit;                // e incrementado erroneamente, logo, temos que subtrair a cada
}                               // nova chama da funcao.
